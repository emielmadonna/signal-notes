...
**Production: https://signal-notes-three.vercel.app** — publicly reachable
(deployment protection is off; the alias always points at the newest deploy of
main).
...
- [ ] https://signal-notes-three.vercel.app loads and you can sign in.
- [ ] Two browser profiles open, each signed in to a DIFFERENT org
      (the card-014 admin accounts — the old *.test demo users were removed):
      - Profile A: admin@admin.admin   (Northwind Advisory)
      - Profile B: admin2@admin.admin  (Meridian Group)
      (password: the SEED_USER_PASSWORD from .env.local)
- [ ] Profile A's workspace shows five COMPLETE briefings; pick the one to
      open in beat 2 NOW, not live (don't pin an id here — pinned briefing ids
      went stale once already, catch #22; the workspace cards are the truth).
...
1. SIGN IN as Profile A → the workspace. One line: "one screen, everything is a sheet
...
5. THE ISOLATION MOMENT (rehearse this): in Profile B, paste the URL of
   Profile A's document or briefing. It renders "This page doesn't exist." — not an
